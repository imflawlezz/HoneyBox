...
actor IndexService {
    ...
    init(storage: StorageService) throws {
        ...
        if let data = storage.readDataIfPresent(at: url) {
            ...
            if loaded.schemaVersion < 2 {
                // v1 → v2: migrate single-slot continue-reading into per-album progress.
    ...
    func homeLatestAlbums(limit: Int) -> [(authorId: String, summary: AlbumSummaryDTO)] {
        let keys = index.recentlyAdded.prefix(limit)
        var out: [(String, AlbumSummaryDTO)] = []
        for entry in keys {
            if let s = albumSummary(authorId: entry.authorId, albumId: entry.albumId) {
    ...
    func homeFavoriteAlbums(limit: Int) -> [(authorId: String, summary: AlbumSummaryDTO)] {
        var out: [(String, AlbumSummaryDTO)] = []
        for key in index.favoriteAlbums {
            let parts = key.split(separator: "/", maxSplits: 1).map(String.init)
            guard parts.count == 2,
                  let s = albumSummary(authorId: parts[0], albumId: parts[1]) else { continue }
    ...
    func continueReadingState() -> ContinueReadingDTO? {
        // Deprecated (schema v1); keep for compatibility.
